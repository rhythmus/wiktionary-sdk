...
**`DecoderRegistry`** and decode orchestration live in **`src/registry/decoder-registry.ts`**. **`registerAllDecoders(reg)`** in **`register-all-decoders.ts`** composes **`registerCoreAndPronunciation(reg)`** from **`register-core-pronunciation.ts`**, then the remaining decoders in historical order (including nested **`registerSectionsAndCitationDecoders`** / **`registerInflectionTableAndStems`** that close over sense helpers). The package entry **`src/registry.ts`** constructs the singleton and calls `registerAllDecoders(registry)`.

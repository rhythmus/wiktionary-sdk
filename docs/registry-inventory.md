...
**`DecoderRegistry`** and decode orchestration live in **`src/registry/decoder-registry.ts`**. **`registerAllDecoders(reg)`** in **`register-all-decoders.ts`** composes **`registerCoreAndPronunciation(reg)`**, **`registerHeadwordsElNlDe(reg)`**, **`registerFormOfWikidata(reg)`**, **`registerTranslations(reg)`**, **`registerSenses(reg)`**, **`registerMorphologyLa(reg)`**, **`registerSemanticRelations(reg)`** (see **`register-headwords-el-nl-de.ts`** … **`register-morphology-la.ts`**, **`register-semantic-relations.ts`**), then etymology, pronunciation-extra, section, and inflection/stem families (still inline in **`register-all-decoders.ts`** until Phase 4.5.10–13). The package entry **`src/registry.ts`** constructs the singleton and calls `registerAllDecoders(registry)`.

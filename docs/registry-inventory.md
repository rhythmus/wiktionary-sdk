...
Ordered list of **`registry.register({ id })`** calls as they appear in source (**registration order is normative**; do not reorder casually). Canonical sequence: **`src/registry/decoder-ids.ts`** (`EXPECTED_DECODER_IDS`) — update it when appending a decoder; **`npm run check:registry-order`** (also prepended to **`npm run test:ci`**) fails on drift. See `STAGED_IMPLEMENTATION_PLAN.md` Phase 4 and `test/registry-ids.test.ts`.

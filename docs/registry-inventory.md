...
**`DecoderRegistry`** and decode orchestration live in **`src/registry/decoder-registry.ts`**. **`registerAllDecoders(reg)`** in **`register-all-decoders.ts`** composes **`registerCoreAndPronunciation(reg)`**, **`registerHeadwordsElNlDe(reg)`**, **`registerFormOfWikidata(reg)`**, **`registerTranslations(reg)`**, **`registerSenses(reg)`** (see **`register-headwords-el-nl-de.ts`**, **`register-form-of-wikidata.ts`**, **`register-translations.ts`**, **`register-senses.ts`**), then the remaining decoders in historical order (including nested **`registerSectionsAndCitationDecoders`** / **`registerInflectionTableAndStems`** that close over **`formatUsageNoteLine`**). The package entry **`src/registry.ts`** constructs the singleton and calls `registerAllDecoders(registry)`.

...
**`DecoderRegistry`** and decode orchestration live in **`src/registry/decoder-registry.ts`**. **`registerAllDecoders(reg)`** in **`register-all-decoders.ts`** composes **`registerCoreAndPronunciation(reg)`**, **`registerHeadwordsElNlDe(reg)`**, **`registerFormOfWikidata(reg)`**, **`registerTranslations(reg)`**, **`registerSenses(reg)`**, **`registerMorphologyLa(reg)`**, **`registerSemanticRelations(reg)`**, **`registerEtymology(reg)`**, **`registerPronunciationExtra(reg)`** (see **`register-pronunciation-extra.ts`** and prior family modules), then section and inflection/stem families (still inline until Phase 4.5.12–13). The package entry **`src/registry.ts`** constructs the singleton and calls `registerAllDecoders(registry)`.

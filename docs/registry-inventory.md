...
**`DecoderRegistry`** and decode orchestration live in **`src/registry/decoder-registry.ts`**. **`registerAllDecoders(reg)`** in **`register-all-decoders.ts`** composes **`registerCoreAndPronunciation(reg)`**, **`registerHeadwordsElNlDe(reg)`**, **`registerFormOfWikidata(reg)`**, **`registerTranslations(reg)`**, **`registerSenses(reg)`**, **`registerMorphologyLa(reg)`**, **`registerSemanticRelations(reg)`**, **`registerEtymology(reg)`**, **`registerPronunciationExtra(reg)`**, **`registerSections(reg)`** (see **`register-sections.ts`** and prior modules), then inflection/stem registration (still inline until Phase 4.5.13). The package entry **`src/registry.ts`** constructs the singleton and calls `registerAllDecoders(registry)`.

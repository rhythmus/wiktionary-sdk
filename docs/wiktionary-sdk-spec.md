...
| **`registry/register-all-decoders.ts`** | **`registerAllDecoders(reg)`** — calls core/pronunciation, then remaining **`reg.register`** blocks in normative order; nested helpers group section/citation and inflection/stem decoders. |

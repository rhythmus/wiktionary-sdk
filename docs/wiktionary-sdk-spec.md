...
| **`registry.ts`** | **`DecoderRegistry`**, **`registry.decodeAll()`**, **`mergePatches()`**, **`stripWikiMarkup()`**, form-of predicates (**`isFormOfTemplateName`**, **`isVariantFormOfTemplateName`**, **`isPerLangFormOfTemplate`**), and all template/section decoders. Ordered decoder list: **`docs/registry-inventory.md`**. |

...
| **`registry.ts`** | **`DecoderRegistry`**, **`registry.decodeAll()`**, and all template/section **`registry.register`** decoders (unchanged order). Re-exports **`stripWikiMarkup`**, form-of predicates, and template sets from **`src/registry/`** (`merge-patches.ts`, `form-of-predicates.ts`, `strip-wiki-markup.ts`). Ordered decoder list: **`docs/registry-inventory.md`**. |

...
-    - Handlebars-based rendering system (Gold Standard v2.3.0).
    ...
    - Inflected form support with morphological redirects ("έγραψα" → "γράφω").
    - Font-neutral fragment architecture for clean UI embedding.
- ✨ **v2.3.0 (Gold Standard - Redirects)**: Introduced morphological redirect views for inflected forms (non-lemma entries), allowing users to see the grammatical profile of a variant before the full lemma record.
 to ensure high-fidelity presentation across playgrounds, CLIs, and web applications.

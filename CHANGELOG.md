...
- **Output schema 3.3.0**: JSON Schema aligned with the TypeScript model (`src/model/` + `src/types.ts` shim) — `schema_version` required on `FetchResult`; `RichEntry` fixes; `AlternativeForm` / `EtymologyData` / `TranslationItem.params` / `debug` / `metadata` tightened; new documentation `$defs` (lexicographic section slugs, langlinks, page metadata, decoder debug, library rows, morphology criteria). `EtymologyData.links` optional deprecated alias of `chain` in TypeScript.
